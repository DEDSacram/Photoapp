...
namespace Photoapp
{
    ...
    public partial class Form1: Form
    {
        ...
        public Bitmap MergeAndClearEdges(Bitmap newBitmap, Bitmap oldBitmap)
        {
            ...
            //    bool continouspixel = false; // pixels are a horizontal line
            ...
            //        // If clearFlag is active, clear the corresponding pixel on the new bitmap
            //        if (clearFlag)
            //        {
            ...
            //int margin = 5; // Margin for allowed consecutive pixels
            ...
            //            // If it's the first non-transparent in the sequence
            //            if (sequenceCount == 1)
            //            {
            ...
            //            // If the sequence length exceeds the margin, disable clearing
            //            if (sequenceCount > margin)
            //            {
            //                clearFlag = false;
            //            }
            //        }
            //        else
            //        {
            //            // Reset the sequence counter if a transparent pixel is found
            //            sequenceCount = 0;
            //        }

            //        // At the end of the row, reset the state
            //        if (x == newBitmap.Width - 1)
            //        {
            //            clearFlag = false;
            //            sequenceCount = 0;
            ...
            //        // If clearFlag is active, clear the pixel on the new bitmap
            //        if (clearFlag)
            //        {
            //            newBitmap.SetPixel(x, y, Color.Transparent);
            //        }
            ...
            for (int y = 0; y < newBitmap.Height; y++)
            {
                ...
                int sequenceCount = 0;
                int pixelCount = 0;
                List<int> clearPositions = new List<int>();
                ...
                for (int x = 0; x < newBitmap.Width; x++)
                {
                    ...
                    {
                        // Reset sequence if a transparent pixel is found
                        sequenceCount = 0;
                    }

                    // Store positions to clear if clearFlag is active
                    if (clearFlag)
                    {
                        clearPositions.Add(x);
                    }
                }

                // If the total non-transparent pixel count is even, proceed to clear
                if (pixelCount % 2 == 0)
                {
                    foreach (int x in clearPositions)
                    {
                        newBitmap.SetPixel(x, y, Color.Transparent);
                    }
                }
            ...
            // Step 2: Union new bitmap with the old one
            for (int y = 0; y < newBitmap.Height; y++)
            {
                for (int x = 0; x < newBitmap.Width; x++)
                {
                    Color pixelA = newBitmap.GetPixel(x, y);
                    Color pixelB = oldBitmap.GetPixel(x, y);

                    // Combine pixels, prioritizing non-transparent pixels
                    if (pixelA.A > 0 || pixelB.A > 0)
                    {
                        result.SetPixel(x, y, Color.Black);  // Edge pixel
                    }
                    else
                    {
                        result.SetPixel(x, y, Color.Transparent);  // Background
                    }
                }
            }
